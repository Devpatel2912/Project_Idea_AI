...
def call_groq(api_key: str, prompt: str, temperature: float = 0.4, max_tokens: int = 8192) -> str:
    ...
    try:
        ...
        raise Exception(f"Groq API returned invalid JSON: {response.text}")

    if not response.ok:
        error_message = data.get("error") or data.get("message") or data.get("msg") or json.dumps(data)
        raise Exception(f"Groq API error {response.status_code}: {error_message}")

    if not isinstance(data, dict):
        raise Exception(f"Unexpected Groq response format: {json.dumps(data, indent=2)[:1000]}")

    if "choices" in data:
        choices = data.get("choices")
        if not isinstance(choices, list) or len(choices) == 0:
            raise Exception(f"Groq returned no choices: {json.dumps(data, indent=2)[:1000]}")
        ...
        raise Exception(f"Unexpected Groq choice format: {json.dumps(choice, indent=2)[:1000]}")
